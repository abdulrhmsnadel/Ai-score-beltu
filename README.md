...
`-x` is for display only; it does not change the scanner logic. Use the HTML/JSON/SARIF output for full report storage.
...
BelTu sends only redacted finding metadata to the optional local model. It does not send cookies, authorization headers, tokens, or raw secrets.
...
A high severity does **not** automatically mean high credibility. The credibility score incorporates severity, engine confidence, verification level, and evidence quality.
...
The release is pinned to stable versions checked against PyPI on **September 12, 2026**: HTTPX 0.28.1, Typer 0.27.2, Rich 15.0.0, Pydantic 2.13.5, PyYAML 6.0.3, pytest 9.1.1, Ruff 0.16.7, and Hatchling 1.32.0. See `docs/DEPENDENCY_BASELINE.md`.
...
Generate a deterministic 10,000-case corpus without making network requests:
...
Network access-control guessing is intentionally capped at 10,000 requests per invocation and does not perform credential/password guessing.

## AI-native v1.5

BelTu now has a task-specific AI router. Each engine is assigned an intentional AI route rather than calling every model blindly.
...
Optional remote providers are opt-in. Configure keys through environment variables; never commit them. Code Llama is treated as a local Ollama model, scikit-learn is used for duplicate/anomaly signals, PentestGPT is planner-only, and the Burp integration is a redacted message bridge.

The historical Claude 3.5 Sonnet model name is accepted only as a compatibility alias and is routed to the current configured Sonnet model because Claude 3.5 Sonnet was retired from the Anthropic API.
...
# Anthropic Claude (current model; old Claude 3.5 name is compatibility-only)
...
AI-SCORE-BELTU now has bounded integrations for OpenAI GPT-4o, Claude, DeepSeek-Coder-V2 compatibility endpoints, Code Llama/Ollama, PentestGPT, Burp message analysis, scikit-learn, LangChain, LangGraph, Interactsh, and PyJWT. Remote AI is opt-in and never replaces deterministic verification.

DeepSeek note: the current public DeepSeek API documents V4 model IDs; the Coder-V2 adapter therefore keeps the model ID configurable for compatible/self-hosted endpoints rather than claiming a current hosted Coder-V2 SKU.
